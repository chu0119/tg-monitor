    ...
    try {
      await Promise.all(
        Array.from(selectedAlerts).map((id) =>
          api.alerts.handle(id, { status, handler_note: note }, "admin")
        )
      );
      fetchAlerts();
            ...
            <div className="flex-1 min-w-0">
              ...
              <p
                ...
                dangerouslySetInnerHTML={{
                  __html: alert.highlighted_message || alert.message_preview
                }}
              />
              {/* 手机端显示时间和状态 */}
      ...
      <div className="space-y-4 max-h-[60vh] sm:max-h-none overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div>
            ...
            <p className="text-sm sm:text-base">{alert.sender_username || "Unknown"}</p>
            {alert.sender_tg_id && (
          ...
          <div>
            ...
            <p className="text-sm sm:text-base">{alert.conversation_title || "Unknown"}</p>
          </div>
        </div>
