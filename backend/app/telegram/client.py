...
class TelegramClientManager:
    ...
    async def create_client(
        ...
        # 规范化代理配置
        normalized_proxy = self._normalize_proxy_config(proxy)
    ...
    async def request_login_code(
        ...
        client = await self.create_client(phone, api_id, api_hash, proxy)
    ...
    async def get_client(self, account_id: int) -> Optional[TelegramClient]:
        ...
        if cached_info:
            ...
            try:
                client = await self.create_client(phone, api_id, api_hash, proxy_config)

                # 确保客户端已连接
                if not client.is_connected():
                    logger.info(f"正在重新连接账号 {account_id}...")
                    await client.connect()

                if await client.is_user_authorized():
                    self.clients[account_id] = client
                    logger.debug(f"账号 {account_id} 客户端已就绪（使用缓存）")
                    return client
                else:
        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                logger.error(f"创建账号 {account_id} 客户端失败: {e}")
                return None
