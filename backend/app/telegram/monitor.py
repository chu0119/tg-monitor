...
class MessageMonitor:
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if conversation.username:
                        ...
                        try:
                            entity = await asyncio.wait_for(
                                client.get_entity(PeerChannel(conversation.chat_id)),
                                timeout=3.0
                            )
                            logger.debug(f"使用 PeerChannel 获取频道实体: {conversation.chat_id}")
                        except Exception:
                            entity = await asyncio.wait_for(
                                client.get_entity(conversation.chat_id),
                                timeout=3.0
                            )
                            logger.debug(f"使用裸 chat_id 获取频道实体: {conversation.chat_id}")
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        # 最后一次尝试失败,使用 PeerChannel 继续监听
                        channel_id = abs(conversation.chat_id)
                        chats = PeerChannel(channel_id)
                        logger.warning(f"频道实体获取最终失败,使用 PeerChannel({channel_id}) 继续监听: {conversation.title}")
    ...
    async def process_message(self, message: Message, conversation_id: int) -> bool:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                await self._broadcast_new_message(db, db_message, sender, conversation_id)
    ...
    async def _broadcast_new_message(self, db, db_message: MessageModel, sender: Sender, conversation_id: int):
        """通过 WebSocket 广播新消息到前端"""
        try:
