...
async def list_alerts(
    ...
    if keyword:
        ...
        if matching_keyword_ids:
            # 构建动态 IN 占位符以兼容 MySQL/aiomysql
            placeholders = ", ".join(f":kw_{i}" for i in range(len(matching_keyword_ids)))
            where_clauses.append(f"a.keyword_id IN ({placeholders})")
            for i, kid in enumerate(matching_keyword_ids):
                params[f"kw_{i}"] = kid
        else:
            return {"items": [], "total": 0, "page": page, "page_size": page_size, "total_pages": 0}

    keyword_group_name = None
    if keyword_group_id:
    ...
    for row in rows:
        ...
        if alert_dict.get("created_at"):
            if isinstance(alert_dict["created_at"], datetime):
                alert_dict["message_date"] = datetime_to_iso(alert_dict["created_at"])
            else:
...
async def get_alert_stats(db: AsyncSession = Depends(get_db)):
    ...
    # 获取趋势数据（最近7天）
    trend_data = []
    # 获取今天的开始时间（UTC naive datetime用于数据库查询）
    from app.utils import start_of_day_local, now_utc
...
async def export_alerts_csv(
    ...
    for alert in alerts:
        ...
        if alert.conversation:
            conversation_title = alert.conversation.title or ""

        # 获取发送者信息
        sender_username = ""
        sender_first_name = ""
        if alert.sender:
            sender_username = alert.sender.username or ""
            sender_first_name = alert.sender.first_name or ""

        row = [
            alert.id,
            format_datetime(alert.created_at) if alert.created_at else "",
            alert.alert_level,
            alert.status,
            alert.keyword_text or "",
            alert.keyword_group_name or "",
            alert.matched_text or "",
            alert.message_id or "",
            alert.conversation_id or "",
            conversation_title,
            alert.sender_id or "",
            sender_username,
            sender_first_name,
            alert.handler or "",
            format_datetime(alert.handled_at) if alert.handled_at else "",
            alert.handler_note or "",
            "是" if alert.notification_sent else "否",
        ]
        writer.writerow(row)
