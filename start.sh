...
start_backend() {
    ...
    # 使用 uvicorn 启动（单进程，Telethon 客户端兼容性更好）
    print_info "使用 Uvicorn 启动后端服务..."
    ...
    if pgrep -f "uvicorn app.main:app" > /dev/null; then
        print_success "后端服务启动成功 (PID: $(pgrep -f 'uvicorn app.main:app' | head -1 | awk '{print $1}'))"
